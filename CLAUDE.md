...
의미있는 코드 수정 완료 시 자동으로 다음 순서 실행:
1. git add 변경된 파일
2. git commit -m "[의미있는 한국어 메시지]"

**git push는 자동 금지.** 사용자가 "푸시해줘", "push" 등 명시 요청 시에만 실행.

### 푸시 거부 시 자동 처리 (push 요청을 받은 경우에만 적용)
...
### 절대 자동 실행 금지
- git push (사용자가 명시 요청 시에만)
- npx vercel --prod (배포 — "배포해줘" 명시 요청 시에만)
- git push --force
- git reset --hard
...
2. 의미있는 단위로 자동 commit (push는 안 함)
3. push는 사용자가 "푸시해줘" 명시 요청 시에만
4. 배포는 사용자가 "배포해줘" 명시 요청 시에만
5. 사용자가 "잘 됐어" "완료" 등으로 확인해도 push/배포는 별도 명시 요청 필요
