...
export default async function handler(req) {
  ...
  // RAG: Supabase에서 최근 fabric_specs 데이터 가져오기
  let ragContext = '';
  try{
    const SUPABASE_URL = process.env.SUPABASE_URL;
    const SUPABASE_KEY = process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_PUBLISHABLE_KEY;
    if(SUPABASE_URL && SUPABASE_KEY){
      const ragRes = await fetch(`${SUPABASE_URL}/rest/v1/fabric_specs?select=category,type,material,color,weight,width,count,price_range&order=id.desc&limit=30`, {
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      if(ragRes.ok){
        ...
        if(ragData && ragData.length > 0){
          ragContext = '\n\n[실제 거래 원단 참고 데이터 - 아래 데이터를 참고하여 더 정확하게 분석하세요]\n';
          ragData.forEach((d, i) => {
            if(d.material || d.weight || d.count){
              ragContext += `${i+1}. ${d.category||''} ${d.type||''} | 소재:${d.material||''} | 번수:${d.count||''} | 중량:${d.weight||''} | 폭:${d.width||''} | 색상:${d.color||''} | 시세:${d.price_range||''}\n`;
            }
          });
        }
      }
    }
  }catch(e){ console.warn('RAG 조회 오류:', e); }
  if (!base64 || !mediaType) {
